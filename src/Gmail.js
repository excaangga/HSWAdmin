...
const Gmail = () => {
    ...
    const [newGmail, setNewGmail] = useState(initialNewDataState);
    const [dirtyData, setDirtyData] = useState({});

    useEffect(() => {
        axios.get(backendUrl + "/social_media_gmail").then((response) => {
            setGmail(response.data);
        });
    }, []);

    const handleNewInputChange = (key, value) => {
        setNewGmail((prevData) => ({ ...prevData, [key]: value }));
    };

    const handleNewArrayInputChange = (field, value) => {
        setNewGmail((prevNewData) => ({
            ...prevNewData,
    ...
    const handleAddData = async () => {
        ...
        try {
            const response = await axios.post(backendUrl + "/social_media_gmail", newGmail);
            const createdData = response.data;
            setGmail((prevData) => [...prevData, createdData]);
            setNewGmail(initialNewDataState);
            axios.get(backendUrl + "/social_media_gmail").then((response) => {
                setGmail(response.data);
            });
        } catch (error) {
    ...
    const handleEditData = (data) => {
        axios.put(backendUrl + "/social_media_gmail/" + data.id, data).then((response) => {
            setGmail([...gmail, response.data]);
            axios.get(backendUrl + "/social_media_gmail").then((response) => {
                setGmail(response.data);
            });
        });
    ...
    const handleDeleteData = (id) => {
        axios.delete(backendUrl + "/social_media_gmail/" + id).then((response) => {
            setGmail(gmail.filter((data) => data.id !== id));
        });
    };
