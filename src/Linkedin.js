...
const Linkedin = () => {
    ...
    const [newLinkedin, setNewLinkedin] = useState(initialNewDataState);
    const [dirtyData, setDirtyData] = useState({});

    useEffect(() => {
        axios.get(backendUrl + "/social_media_linkedin").then((response) => {
            setLinkedin(response.data);
        });
    }, []);

    const handleNewInputChange = (key, value) => {
        setNewLinkedin((prevData) => ({ ...prevData, [key]: value }));
    };

    const handleNewArrayInputChange = (field, value) => {
        setNewLinkedin((prevNewData) => ({
            ...prevNewData,
    ...
    const handleAddData = async () => {
        ...
        try {
            const response = await axios.post(backendUrl + "/social_media_linkedin", newLinkedin);
            const createdData = response.data;
            setLinkedin((prevData) => [...prevData, createdData]);
            setNewLinkedin(initialNewDataState);
            axios.get(backendUrl + "/social_media_linkedin").then((response) => {
                setLinkedin(response.data);
            });
        } catch (error) {
    ...
    const handleEditData = (data) => {
        axios.put(backendUrl + "/social_media_linkedin/" + data.id, data).then((response) => {
            setLinkedin([...linkedin, response.data]);
            axios.get(backendUrl + "/social_media_linkedin").then((response) => {
                setLinkedin(response.data);
            });
        });
    ...
    const handleDeleteData = (id) => {
        axios.delete(backendUrl + "/social_media_linkedin/" + id).then((response) => {
            setLinkedin(linkedin.filter((data) => data.id !== id));
        });
    };
